...
ローカル環境起動中に `main.py` などのソースコードを変更した場合、Docker Composeの `volumes` 設定と `--reload` オプションにより、**コードの変更が即座にAPIサーバーに反映（ホットリロード）されます。**

コードを保存したらすぐにブラウザ（Swagger UI）を更新して、変更後の挙動をテストすることができます。

### 🚨 もし変更が反映されない・エラーが出る場合
万が一、「コードを変えたのに挙動が変わらない」「APIが 404 Not Found などになる」といった場合は、以下のコマンドでAPIコンテナのみを再起動して反映させてください。
```bash
docker compose restart api
```
（※DynamoDBのデータは維持されたまま、APIサーバーのみがクリーンに再起動します）
