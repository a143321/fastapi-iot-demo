...
💡 **画面が古いまま更新されない場合（ブラウザキャッシュ対策）**
デプロイ（ECSの再起動）が完了しているにもかかわらず、ブラウザで本番URLにアクセスすると古いUIのまま表示されることがあります。これはブラウザが静的ファイル（`index.html` など）をローカルにキャッシュしているための正常な仕様です。
サーバーから最新のファイルを再取得させるために、以下の**「スーパーリロード（ハードリロード）」**を実行してください。
