...
モダンな開発プロセスにおいて不可欠なテストと自動化のベストプラクティスです。

### 4.1. FastAPIにおける単体テスト (TestClient)
FastAPIには標準で `TestClient` が用意されており、サーバーを実際に起動しなくてもAPIの挙動をテストできます。`pytest` などのテストフレームワークと組み合わせて使用します。
...
def test_sensor_data_validation():
    # 絶対零度未満の不正なデータを送信
    payload = {"temperature": -300.0, "humidity": 50.0, "occupancy": True}
    response = client.post("/api/sensors/device-123/data", json=payload)
    
    # バリデーションで正しく弾かれているか（422エラー）を確認
    assert response.status_code == 422
```
これにより、Pydanticの型チェックや `extra="forbid"` の制約が正しく機能しているかを機械的に担保（TDD）できます。

### 4.2. CI/CDパイプライン (GitHub Actions)
書かれたテストコードが常にパスすることを保証するため、GitHub Actionsを利用したCIパイプラインを構築します。
`.github/workflows/ci.yml` を定義することで、コードがPushされるたびにクラウド上で自動テストが走る仕組みです。

- **自動化のメリット**: チーム開発において、他の開発者が壊れたコードをPushしてしまった場合でも、マージ前に気づくことができ、プロダクションの品質が保たれます。
